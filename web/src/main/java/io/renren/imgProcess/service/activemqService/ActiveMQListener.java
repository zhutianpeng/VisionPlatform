...
//              比较task列表并分发存入对应的redis的list
    ...
    @JmsListener(destination = "realTime2DInput", containerFactory = "queueListenerFactory")
    public void realTime2DOnMessage(Message message) {
        if (message instanceof BytesMessage) {
            BytesMessage bytesMessage = (BytesMessage)message;
            try {
                // 得到一些参数：
...
//              比较task列表并分发存入对应的redis的list
    ...
    @JmsListener(destination = "realTime3DInput", containerFactory = "queueListenerFactory")
